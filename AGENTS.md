...
- `docs/PRD.md` 定义产品范围；根目录 `task.json` 只保存当前阶段，是活动任务状态的唯一事实来源；`docs/tasks/*.json` 是已结束阶段的只读快照，不可领取或更新；`process.txt` 是只追加的执行日志。任务文件中的开发状态不是产品中的异步 Job 状态。
- 先细化当前阶段；后续阶段保留 PRD 目标，阶段验收后再拆任务。不得自行扩展产品范围或降低验收标准。
- 一个任务对应一个可独立验证、可审查的结果，包含 ID、依赖、范围、验收标准和验证步骤。复杂任务先拆分；实现、必要测试、迁移和相关文档应一起交付。
- 可执行任务使用 `MVP-0.2-T001` 这类“阶段 + 序号”格式。一个任务只交付一个明确行为，通常有 2–5 条验收条件；若出现多个独立行为、需要多个完成提交或无法一次验证，先拆分任务并更新依赖。
- 阶段退出验收完成后，先把最终根 `task.json` 原样归档为 `docs/tasks/<phase>.json`，再用下一阶段的细化任务替换根文件；归档与切换应在同一治理提交中完成。不得提前维护活动阶段的重复副本。
...
3. 在 `codex/<task-or-stage>` 功能分支开发，禁止自动向 main/master 提交或合并。
   - 同一执行者连续完成同一阶段或同一批次的串行任务时，可以复用一个功能分支，无需每个任务新建分支。
   - 复用分支不改变任务粒度：每个任务仍须独立领取、验证、更新 `task.json` 和 `process.txt`，并创建包含 `Task-Completed` 的完成提交。
   - 并行开发、需要独立评审或回滚、任务之间没有直接依赖时，为每个任务创建独立分支。
   - 多人协作时，先推送包含前置任务的共享基线；后续执行者从该远程分支创建自己的任务分支，不直接在 main/master 上开发。
   - 分支名可以使用任务、任务范围或阶段，例如 `codex/mvp-0.2-t006`、`codex/mvp-0.2-t001-t005`、`codex/mvp-0.2`。
...
MVP 0.1 已通过真实 DeepSeek Provider、浏览器主链路和首段性能退出验收。项目进入 MVP 0.2，当前重点是 10 节点知识结构、渐进画像、服务端测验评分、掌握度更新与可解释下一步推荐；活动任务以根目录 `task.json` 为准。
